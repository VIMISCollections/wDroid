...
echo "Setting up app..."
adb -s $1 uninstall $3
adb -s $1 shell rm -r /data/local/tmp/$3_report
adb -s $1 install $5
echo "Running monkey..."
adb -s $1 shell /data/local/tmp/monkey -n $2 -t $4 -p $3 -v -v -v --ignore-crashes --ignore-timeouts --kill-process-after-error --ignore-security-exceptions --pct-syskeys 0 --pct-nav 0 99999 | tee $6/$3.txt
echo "Getting bug report from device..."
rm -r $6/$3_report
adb -s $1 pull /data/local/tmp/$3_report $6
echo "Clearing phone..."
adb -s $1 shell rm -r /data/local/tmp/$3_report
